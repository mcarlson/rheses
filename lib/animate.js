...
(function(){
	...
	this.timestep = function(time_stamp, first_value, time_skew){
		...
		if (track.repeat < 0){
			if(norm_time >= -track.repeat) end_gap = track.repeat + norm_time, norm_time = -track.repeat
			norm_time = norm_time % 2
			if(norm_time > 1) norm_time = 2 - norm_time
		}
		else if (track.repeat > 0){
		...
		var eased_time = track.easing(norm_time, track.control)
		if(eased_time < 0.001 ) eased_time = 0
		if(eased_time > 0.999 ) eased_time = 1
	...
	this.update = function(track){
		...
		if(!track.easing && old_track) track.easing = this.current.easing
	...
	this.buildTrack = function(track){
		...
		var mytrack = {
			...
			easing: easing[track.easing || (track.control?'bezier':'linear')] || easing.linear, // fail silently on wrong easing function
			speed: track.speed ||  1,
		...
		if(track.control) mytrack.control = prepBezier(track.control, mytrack.last_time)
