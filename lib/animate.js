...
(function(){
	...
	this.timestep = function(time_stamp, first_value, time_skew){
		...
		var norm_time = (local_time / track.last_time) * track.speed

		// lets check the looping
		var end_gap = undefined

		if (track.times < 0){
			if(norm_time >= -track.times) end_gap = track_times + norm_time, norm_time = -track.times
			norm_time = norm_time % 2
			if(norm_time > 1) norm_time = 1 - norm_time
		}
		else if (track.times > 0){
			if(norm_time >= track.times) end_gap = norm_time - track.times, norm_time = track.times - 0.000001
		...
		if(end_gap !== undefined){ // alright we are stopping
			var end_ms = end_gap * track.last_time
		...
		var eased_time = track.ease(norm_time, track.bezier)
		// lets convert it back to keyspace time
		var track_time = eased_time * track.last_time
		...
		if(key1 !== undefined){
			...
			if(key2 === undefined) this7.value = key1 in track.values? track.values[key1]: track.first_value
			else{ // interpolate
				var inter_key_time
	...
	this.update = function(track){
		...
		if(!track.ease && old_track) track.ease = this.current.ease
	...
	this.buildTrack = function(track){
		...
		var mytrack = {
			...
			times: track.times !== undefined? track.loop: 1,
			ease: easing[track.ease || (track.bezier?'bezier':'linear')] || easing.linear, // fail silently on wrong easing function
			speed: track.speed ||  1,
		...
		for(var key in track){
			if(parseFloat(key) == key){
				...
				if(key > mytrack.last_time) mytrack.last_time = key
			}
		}
		if(track.bezier) mytrack.bezier = prepBezier(track.bezier, mytrack.last_time)
	...
	function prepBezier(bezier, time){
		var b = {}
		b.epsilon = 1.0/(200.0*time)
		b.points = bezier
		b.cx = 3.0*bezier[0]
		b.bx = 3.0 * (bezier[2] - bezier[0]) -b.cx
		b.ax = 1.0 - b.cx - b.bx
		b.cy = 3.0 * bezier[1]
		b.by = 3.0 * (bezier[3] - bezier[1]) - b.cy
		b.ay = 1.0 - b.cy - b.by
		return b
	}
	...
	// standard jquery easing functions
	var easing = {
		bezier:function(t, bezier) {
			// Given an x value, find a parametric value it came from.
			return bezierCurveY(bezierSolveX(t, bezier), bezier)
		},
		bret:function(t){ // get the curve
			// pick a d that seems to make sense
			var d = 0.1
			var Xi = 0 // we go from 0 
			var Xf = 1 // to 1 , as we are an easing function
			var Xn = Xf + d // here too
			var Xo = Xi - d // as per email
			// compute the constant
			var K = (Xo - Xf) * (Xi - Xn) / (Xo - Xi) * (Xf - Xn)
			// seems to be 1.20001
			var Kt = Math.pow(K, t)
			// so when t starts at 0 and ends at 1 K(t) is just K^t
			return Xo *(Xi - Xn) + Kt * Xn * (Xo - Xi) / (Xi - Xn + Kt * (Xo - Xi)) 
		},
		...
		inBounce:function(t){ return 1 - easing.outBounce (1-t) },
