...
(function(){
	...
	this.serialize_color = function(){
		return '#' + ('000000' + ( ( ( ( ( this[0]*255 ) & 0xff ) << 8 | ( ( this[1]*255 ) & 0xff ) ) << 8 ) | ( ( this[2]*255 ) & 0xff ) ).toString( 16 ) ).slice( -6 )
	}
	...
	this.playStateless = function(track){
	...
	this.parse_color = function( col ) {
		...
		a.toString = this.serialize_color
		if( c === undefined ){

			if(col.charCodeAt(0) == 35) col = col.slice(1)
			...
			while(i<len){
				...
				if(ch >= 48 && ch <= 57){
					...
					col = col.toLowerCase()
					c = this.color_lookup[col]
					if(c === undefined) for(var k in this.color_lookup){
						if(k.indexOf(col) != -1){
							c = this.color_lookup[k]
							// cache it
							this.color_lookup[col] = c
							break
						}
					}
					len = 0
