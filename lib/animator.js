...
(function(){
	...
	this.timestep = function(time_stamp, first_value, time_skew){
		...
		if(track.start !== undefined){
			if(local_time - track.start <= 0) local_time = 0
			else local_time -= track.start
		}
		...
		var track_time = motion_time * track.last_time
	...
	this.buildTrack = function(track){
		...
		var mytrack = {
			...
			start: track.start !== undefined? track.start: 0,
			onend: track.onend,
