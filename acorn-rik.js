...
define(function(require, exports, module){
	...
	var defaultOptions = exports.defaultOptions = {
		...
		sourceFile: null,
		// Token options
		// dont close the tokentree tags: )'s and ]'s are missing
		noclose:false,
		// compact symbol rep, dont split n.x.y 
		compact:false,
		// tokens
		tokens:false,
	};
	...
	function finishToken(type, val) {
		...
		// output a token in the token tree
		if(options.tokens){
			var n
			if(type == _eof) return
			...
			if(type == _braceL || type == _bracketL || type == _parenL){
				...
				/*var ptype = tokTree._t
				if(type == _braceR && ptype != _braceL || 
					type == _bracketR && ptype != _bracketL ||
					type == _parenR && ptype != _parenL){
					console.log('assymetrical tree at ',n.y,n.x,ptype)
					process.exit(0)
				}
				*/
				if(options.noclose){
	...
	function setStrict(strct) {
		strict = strct;
		if(!options.tokens){ // dont do this in tokentree mode
			tokPos = lastEnd;
			skipSpace();
			readToken();
		}
